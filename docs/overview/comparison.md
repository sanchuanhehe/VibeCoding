...
| Claude Code | Agentic coding（终端/IDE/桌面/Web） | Terminal / VS Code / JetBrains / Desktop / Web | 代码库级操作、命令执行、MCP 连接、可用 GitHub Actions/GitLab CI 做自动化 提出了很多领先的理念,例如skills mcp 插件 subagent,已经成为ai coding届的事实标准 虽然闭源,但靠着优秀的sdk和众多领先的理念和,是生态最优秀的工具 | 需要“一套引擎多端一致体验”的团队 | 高级能力常依赖订阅与账号体系；不同端能力细节有差异 |
