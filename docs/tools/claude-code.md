...
## 云端与 GitHub 集成 (Claude Code on the Web)

除了完全运行在本地，Claude Code 还提供了云端环境平台并深度集成了 GitHub：

1. **GitHub App 集成使用**
   - 开发者可以将 Claude GitHub 应用程序与其仓库连接，通过访问 `claude.ai/code` 发起特定开发任务。
   - 这使得开发者能够直接将修复构建推送到远程分支，并通过内置的 Diff 视图在生成 Pull Request 前反复评审和优化代码。

2. **本地与云端无缝切换 (Terminal to Web)**
   - 开发人员可以通过终端的 `claude --remote "任务指令"` 命令在 Anthropic 的云设施上启动独立的代理会话，不占用本地资源且支持后台并行化运行多项任务。
   - 通过 `claude --teleport` 等指令又可以将云端的会话无缝“传送 (Teleport)”回本地终端接管以继续工作。

3. **安全的云端沙箱环境**
   - 执行 Web 会话时，Claude 将在一个 Anthropic 托管的隔离虚拟机内加载一个包含多种现代开发流所需的 Universal Image（涵盖 Node, Python, JVM, DB等配置）。
   - 提供专用安全代理。包含对于 GitHub Git 客户端权限的代理管控，防止敏感秘钥接触沙箱环境，同时所有外部流量都需要通过预设白名单网络代理访问。

## 适合场景

- 需要“一套引擎多端一致体验”的团队

> 笔者注,最近cc推出了remote-control功能,可以让用户在本地使用claude code的能力控制远程服务器上的代码,这对于需要在远程环境中进行开发的用户来说是一个非常实用的功能,进一步拓展了claude code的适用场景(其实某种程度上和openclaw差不多)

## 主要限制/注意点

- 高级能力常依赖订阅与账号体系；不同端能力细节有差异
- 有些prompt针对claude模型进行了优化，其他模型可能无法完全发挥效果
...
- 功能概览：<https://code.claude.com/docs/en/overview>
- 核心工作原理：<https://code.claude.com/docs/en/how-claude-code-works>
- 拓展生态概览：<https://code.claude.com/docs/en/features-overview>
- Web 版与 GitHub 集成：<https://code.claude.com/docs/en/claude-code-on-the-web>
